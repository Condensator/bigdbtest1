...
INSERT INTO ReversalReceivableDetail_Extract (ReceivableTaxId, IsCashPosted,ReceivableTaxDetailId, ReceivableDetailId, Currency,TaxAreaId, Cost,
		...
		SalesTaxRemittanceResponsibility,IsAssessSalesTaxAtSKULevel,UpfrontTaxAssessedInLegacySystem,ReceivableTaxType)
SELECT  RT.Id AS ReceivableTaxId
		...
		,CASE WHEN (RecType.Name = @BuyOut) THEN @SALE ELSE @LEASE END AS TransactionType
...
WHERE HasGLPeriod = 0 AND R.JobStepInstanceId = @JobStepInstanceId
...
INNER JOIN LeaseAssets LA ON LA.LeaseFinanceId = LF.Id AND LA.AssetId = RD.AssetId AND LA.IsActive = 1
