...
GO
CREATE PROC [dbo].[GetReceivableDetailsForCustomer]
(
@ContractTableForCustomer ContractIdTableTypeForCustomer READONLY,
...
R.SourceTable,
R.PaymentScheduleId
FROM Receivables R JOIN @ContractTableForCustomer CT ON R.CustomerId = CT.CustomerId
WHERE
(R.ID BETWEEN CT.ReceivableStartId AND ReceivableEndId)
AND R.DueDate <= CT.DueDate
AND R.IsActive = 1
AND CT.CustomerId = R.CustomerId
AND ((CT.ContractId IS NULL AND R.EntityType = 'CU' )
OR (CT.ContractId = R.EntityId AND R.EntityType = 'CT') OR
(CT.DiscountingId = R.EntityId AND R.EntityType = 'DT'))
;
SELECT
R.Id ReceivableId,
...
INNER JOIN Locations L ON AL.LocationId = L.Id JOIN @ContractTableForCustomer CT ON L.CustomerId = CT.CustomerId
;
SELECT
MAXAL.AssetId
,MAXAL.DueDate
...
Loc.UpfrontTaxMode
INTO #AllTaxAreaIdForLocation
FROM LocationTaxAreaHistories p
INNER JOIN #ReceivableIds RId ON RId.LocationId = P.LocationId
JOIN @ContractTableForCustomer CT on CT.CustomerId = RId.CustomerId
INNER JOIN Locations Loc ON RId.LocationId = Loc.id
;
SELECT *
INTO #TaxAreaId
FROM #AllTaxAreaIdForLocation
WHERE Row_Num = 1;
SELECT * INTO  #FromTaxAreaId
FROM #AllTaxAreaIdForLocation
WHERE Row_Num = 2
SELECT
...
LEFT JOIN #TaxAreaId TAID ON R.LocationId = TAID.LocationId
LEFT JOIN SaleLeasebackCodeConfigs SLBC ON A.SaleLeasebackCodeId = SLBC.Id
LEFT JOIN SalesTaxExemptionLevelConfigs STELC ON A.SalesTaxExemptionLevelId = STELC.Id
...
LEFT JOIN #FromTaxAreaId FromTAID ON R.LocationId = FromTAID.LocationId
...
,RD.ReceivableId
,RD.ContractId
INTO #RentalReceivableWithStateDetails
...
,RD.StateId
,RD.AssetId
,ISNULL((SELECT ISNULL(SUM(BRR.RevenueBilledToDate_Amount),0.00) FROM VertexBilledRentalReceivables BRR
...
FROM #RentalReceivableWithStateDetails RD
;
SELECT
...
FROM
LeaseFinances LF
JOIN @ContractTableForCustomer CT ON LF.CustomerId = CT.CustomerId
...
;
SELECT *
INTO #LeaseAssetLocationsAsOfDueDate
...
FROM
#LeaseAssetLocationsAsOfDueDate AssetLoc
JOIN @ContractTableForCustomer CT ON CT.ContractId = AssetLoc.ContractId
LEFT JOIN LocationTaxAreaHistories p ON P.LocationId = AssetLoc.LocationId
;
SELECT * INTO #TaxAreaIdAsOfDueDate
...
CAST(CASE WHEN countries.TaxSourceType = 'Vertex' THEN 1 ELSE 0 END AS BIT) 'IsVertexSupportedLocation',
CTE.ContractId,
ISNULL(L.JurisdictionId,0) 'TaxJurisdictionId'
FROM
#TaxAreaIdAsOfDueDate CTE
