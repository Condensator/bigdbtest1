...
CREATE PROCEDURE [dbo].[NonRentalInvoiceReport]
(
@InvoiceId BIGINT
) WITH RECOMPILE
AS
...
DECLARE @BillToId BIGINT,@GenericInvoiceComment nvarchar(200),@TotalAmount decimal(16,2),@DueDate Date;
...
AND (th.AssetID IS NULL OR th.AssetID = tax.AssetID)
...
BlendRentalAmount ,
BlendNumber ,
DetailId into #BlendDetails from GetBlendDetails(@InvoiceID,@BillToId) AS blenddetails

Select @TotalAmount= Sum(BlendRentalAmount) from #BlendDetails
...
SELECT * INTO #TempResult FROM CTE_Final;
;WITH CTE_GST AS
(
...
,(@TotalAmount * receivable.ExchangeRate) + ISNULL(CTE_SalesTax.SalesTax,0.00) + ISNULL(CTE_GST.GSTHST,0.00)+ISNULL(CTE_QST.QST,0.00)+ISNULL(CTE_PST.PST,0.00)  [TotalAmountDue]
