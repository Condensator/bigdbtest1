...
CREATE PROCEDURE [dbo].[ReceiptHistoryReport]
(
@FromDate DATETIMEOFFSET,
@ToDate DATETIMEOFFSET,
@Currency NVARCHAR(3) = NULL,
@CustomerNumber NVARCHAR(80) = NULL,
@ReceiptType NVARCHAR(60) = NULL,
@IsSummaryReport BIT,
...
ReceiptBatch NVARCHAR(80),
Status NVARCHAR(30),
CheckNumber NVARCHAR(80),
...
Receipts.CustomerId AS CustomerId,
Receipts.EntityType AS EntityType,
Receipts.ContractId AS ContractId
INTO #ValidReceipts
FROM
Receipts
...
Parties.PortfolioId AS PortfolioId
...
WHERE ((@CustomerNumber IS NULL AND Parties.PortfolioId=@CurrentPortfolioId) OR Parties.PartyNumber = @CustomerNumber)
GROUP BY
#ValidReceipts.ReceiptId,
Receivables.CustomerId,
Parties.PartyNumber,
...
INSERT INTO #CustomerLevelCharges
SELECT 
#ValidReceipts.ReceiptId,
#ValidReceipts.CustomerId,
CAST(0.00 AS Decimal(16,2)) AS Charges,
CAST(0.00 AS Decimal(16,2)) AS Taxes,
Parties.PartyNumber AS CustomerNumber,
Parties.PartyName AS Customer,
Parties.PortfolioId AS PortfolioId
FROM #ValidReceipts
JOIN Parties ON #ValidReceipts.CustomerId = Parties.Id
WHERE CheckAmount = UnallocatedCashBalance
AND ((@CustomerNumber IS NULL AND Parties.PortfolioId=@CurrentPortfolioId) OR Parties.PartyNumber = @CustomerNumber)
AND #ValidReceipts.EntityType='Customer'
...
Parties.PortfolioId AS PortfolioId
FROM #ValidReceipts
JOIN LeaseFinances LF ON LF.ContractId = #ValidReceipts.ContractId AND LF.IsCurrent = 1
JOIN Parties ON Parties.Id  = LF.CustomerId
WHERE CheckAmount = UnallocatedCashBalance
AND ((@CustomerNumber IS NULL AND Parties.PortfolioId=@CurrentPortfolioId) OR Parties.PartyNumber = @CustomerNumber)
AND #ValidReceipts.EntityType ='Lease'
...
Parties.PortfolioId AS PortfolioId
FROM #ValidReceipts
JOIN LoanFinances LF ON LF.ContractId = #ValidReceipts.ContractId AND LF.IsCurrent = 1
JOIN Parties ON Parties.Id  = LF.CustomerId
WHERE CheckAmount = UnallocatedCashBalance
AND ((@CustomerNumber IS NULL AND Parties.PortfolioId=@CurrentPortfolioId) OR Parties.PartyNumber = @CustomerNumber)
AND #ValidReceipts.EntityType ='Loan'
...
WHERE ((@CustomerNumber IS NULL AND #CustomerLevelCharges.PortfolioId=@CurrentPortfolioId) OR #CustomerLevelCharges.CustomerNumber = @CustomerNumber)
GROUP BY
Receipts.ReceiptId,
#CustomerLevelCharges.CustomerId,
Receipts.Currency,
