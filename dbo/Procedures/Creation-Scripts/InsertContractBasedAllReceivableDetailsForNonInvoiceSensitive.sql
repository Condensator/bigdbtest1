...
CREATE PROCEDURE [dbo].[InsertContractBasedAllReceivableDetailsForNonInvoiceSensitive]
(
...
FROM
Receivables R
INNER JOIN Contracts CT ON R.EntityId = CT.Id AND CT.ContractType = @ContractType AND R.EntityType = @EntityType
...
WHERE
R.IsActive =1 AND RD.IsActive =1 AND RD.IsTaxAssessed = 0
AND (R.DueDate <= @ComputedProcessThroughDate) AND (@ExcludeBackgroundProcessingPendingContracts = 0 OR CT.BackgroundProcessingPending = 0);
