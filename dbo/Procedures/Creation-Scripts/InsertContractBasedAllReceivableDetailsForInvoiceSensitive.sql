...
CREATE PROCEDURE [dbo].[InsertContractBasedAllReceivableDetailsForInvoiceSensitive]
(
...
;WITH CTE_ContractsToBeProcessed AS
(
SELECT
CT.Id AS ContractId
...
Contracts CT
LEFT JOIN
ContractBillings CB ON CB.Id = CT.Id AND CT.ContractType = @ContractType
...
FROM
Receivables R
INNER JOIN CTE_ContractsToBeProcessed C ON R.EntityId = C.ContractId  AND R.EntityType = @EntityType
