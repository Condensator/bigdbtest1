...
CREATE PROCEDURE [dbo].[InsertSalesTaxReversalDiscountingBasedReceivableDetails]
(
...
INSERT INTO ReversalReceivableDetail_Extract (ReceivableTaxId, IsCashPosted,ReceivableTaxDetailId, ReceivableDetailId, Currency,TaxAreaId, Cost,
		...
		TransactionType, ReceivableType, IsRental, CustomerId, ContractId, DiscountingId, LegalEntityId, EntityType, IsVertexSupported, ReceivableDetailRowVersion,
		ReceivableTaxRowVersion, ReceivableTaxDetailRowVersion, AssetLocationRowVersion,JobStepInstanceId,CreatedById, CreatedTime,UpfrontTaxSundryId, SalesTaxRemittanceResponsibility, 
		IsAssessSalesTaxAtSKULevel, UpfrontTaxAssessedInLegacySystem,ReceivableTaxType,BusCode)
SELECT 	RT.Id AS ReceivableTaxId
		...
		,CASE WHEN R.EntityType = @DTEntityType THEN @DiscountingId ELSE NULL END AS DiscountingId
...
AND R.EntityType = @DTEntityType
AND R.EntityId = @DiscountingId;
UPDATE ReversalReceivableDetail_Extract
SET ErrorCode = @InvoicedOrCashPosted
WHERE JobStepInstanceId = @JobStepInstanceId AND (IsInvoiced = 1 OR IsCashPosted = 1);
WITH CTE_DistinctLegalEntities AS
(
Select DISTINCT LegalEntityId FROM ReversalReceivableDetail_Extract R
WHERE R.IsInvoiced = 0 AND R.IsCashPosted = 0 AND JobStepInstanceId = @JobStepInstanceId
)
...
WHERE L.HasGLPeriod = 0 AND JobStepInstanceId = @JobStepInstanceId
UPDATE R
...
FROM ReversalReceivableDetail_Extract R
INNER JOIN #LegalEntitiesInfo L ON R.LegalEntityId = L.LegalEntityId
WHERE HasGLPeriod = 1 AND IsPostDateValid = 0 AND JobStepInstanceId = @JobStepInstanceId
