...
CREATE PROC [dbo].[SaveLoanPayDownOutStandingChargeDetail]
(
 @val [dbo].[LoanPayDownOutStandingChargeDetail] READONLY
)
AS
SET NOCOUNT ON;
...
MERGE [dbo].[LoanPayDownOutStandingChargeDetails] With (ForceSeek) AS T
...
WHEN NOT MATCHED THEN
	INSERT ([CreatedById],[CreatedTime],[DueDate],[IncludeinInvoice],[IsActive],[LoanPaydownId],[ReceivableAmount_Amount],[ReceivableAmount_Currency],[ReceivableBalance_Amount],[ReceivableBalance_Currency],[ReceivableId],[ReceivableType],[SalesTaxAmount_Amount],[SalesTaxAmount_Currency],[SalesTaxBalance_Amount],[SalesTaxBalance_Currency])
    VALUES (S.[CreatedById],S.[CreatedTime],S.[DueDate],S.[IncludeinInvoice],S.[IsActive],S.[LoanPaydownId],S.[ReceivableAmount_Amount],S.[ReceivableAmount_Currency],S.[ReceivableBalance_Amount],S.[ReceivableBalance_Currency],S.[ReceivableId],S.[ReceivableType],S.[SalesTaxAmount_Amount],S.[SalesTaxAmount_Currency],S.[SalesTaxBalance_Amount],S.[SalesTaxBalance_Currency])
