...
CREATE PROC [dbo].[SaveLeaseInsuranceRequirement]
(
 @val [dbo].[LeaseInsuranceRequirement] READONLY
)
AS
SET NOCOUNT ON;
...
MERGE [dbo].[LeaseInsuranceRequirements] With (ForceSeek) AS T
...
WHEN NOT MATCHED THEN
	INSERT ([AggregateAmount_Amount],[AggregateAmount_Currency],[AggregateDeductible_Amount],[AggregateDeductible_Currency],[CoverageTypeConfigId],[CreatedById],[CreatedTime],[IsActive],[IsContractAmount],[IsManual],[LeaseFinanceId],[PerOccurrenceAmount_Amount],[PerOccurrenceAmount_Currency],[PerOccurrenceDeductible_Amount],[PerOccurrenceDeductible_Currency],[Status])
    VALUES (S.[AggregateAmount_Amount],S.[AggregateAmount_Currency],S.[AggregateDeductible_Amount],S.[AggregateDeductible_Currency],S.[CoverageTypeConfigId],S.[CreatedById],S.[CreatedTime],S.[IsActive],S.[IsContractAmount],S.[IsManual],S.[LeaseFinanceId],S.[PerOccurrenceAmount_Amount],S.[PerOccurrenceAmount_Currency],S.[PerOccurrenceDeductible_Amount],S.[PerOccurrenceDeductible_Currency],S.[Status])
