...
CREATE PROC [dbo].[ExtractApplicationGroupedInvoices] 
(
...
(
  Id BIGINT,
	...
	--For IsFullPosting=1 AND IsGrouped=1, Insert into ReceiptApplicationReceivableDetails_Extract

	SELECT 
		RPBF.InvoiceNumber Number, 
		RPBF.ComputedReceivableInvoiceId ReceivableInvoiceId, 
		RPBF.ReceiptAmount,
		(RID.EffectiveBalance_Amount - ISNULL(RDWTH.EffectiveBalance_Amount, 0.00)) AmountApplied, 
		RID.EffectiveTaxBalance_Amount TaxApplied, 
	...
	WHERE RPBF.JobStepInstanceId = @JobStepInstanceId
		AND RPBF.HasError = 0
		AND RPBF.ComputedIsFullPosting = 1
		AND RPBF.ComputedIsGrouped = 1
		AND RPBF.ComputedIsDSL = 0
		AND RPBF.NonAccrualCategory IS NULL
		AND RPBF.IsInvoiceInMultipleReceipts = 0 AND RPBF.CreateUnallocatedReceipt=0
	...
	SELECT Number, ReceivableInvoiceId, ReceiptAmount, ReceivableDetailId, DumpId FROM #RRDExtract
	...
	;
		...
		INSERT INTO ReceiptApplicationReceivableDetails_Extract (AmountApplied, TaxApplied, BookAmountApplied, 
		ReceivableDetailId, ReceivableDetailIsActive, InvoiceId, ContractId, DiscountingId, ReceivableId, 
		ReceiptId, JobStepInstanceId, ReceiptApplicationReceivableDetailId, DumpId, CreatedById, CreatedTime,
		IsReApplication,LeaseComponentAmountApplied,NonLeaseComponentAmountApplied)
