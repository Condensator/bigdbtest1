...
CREATE PROCEDURE [dbo].[InsertCustomerBasedReceivableDetailsForNonInvoiceSensitive]
(
...
WHERE
R.IsActive =1 AND RD.IsActive =1 AND R.CustomerId = @CustomerId
AND RD.IsTaxAssessed = 0 AND (R.DueDate <= @ComputedProcessThroughDate) AND BPPC.Id IS NULL 
