...
CREATE PROCEDURE [dbo].[ExtractDataForInvoiceGeneration_Contract] (
...
BEGIN
	...
	--This can be removed as Invoice generation happens on Payoff/Paydown and we already have a check for BilledStatus in the below Select Statement
 ...
 WHILE 1=1  
 BEGIN  
 SELECT TOP (@JobPaginationCount) R.Id RId, RD.Id RDId, rd.BillToId,ISNULL(CPUB.InvoiceLeaddays,CB.InvoiceLeaddays) As InvoiceLeadDays,  
 ISNULL(CPUB.InvoiceTransitDays, CB.InvoiceTransitDays) AS InvoiceTransitDays   
 INTO #Receivables  
 ...
 INNER JOIN ReceivableDetails RD WITH (FORCESEEK) ON R.Id = RD.ReceivableId
	...
	INNER JOIN ContractBillings CB ON C.Id = CB.Id
		AND CB.IsActive = 1
	LEFT JOIN CPUSchedules CPUS ON R.SourceId = CPUS.Id 
		AND R.SourceTable = @ReceivableSourceTable_CPUSchedule
	LEFT JOIN CPUBillings CPUB ON CPUS.CPUFinanceId = CPUB.Id
	LEFT JOIN #PreviousJobBillToIds P ON rd.BillToId=P.BillToId
	WHERE RD.ID > @MaxReceivableDetailId AND R.DueDate <= CASE WHEN @IsInvoiceSensitive = 1
							THEN DATEADD(DD, ISNULL(CPUB.InvoiceLeaddays, CB.InvoiceLeaddays), @ProcessThroughDate)
		...
		AND (
			(R.IsDummy = 1 AND R.IsDSL = 1)	OR R.IsDummy = 0
			)
		AND (
			@ContractId IS NULL
			OR (
		...
		AND (
			@IsAllLease = 0	OR C.ContractType = @ContractFilterEntityType_Lease
			)
		AND (
			@IsAllLoan = 0 OR C.ContractType = @ContractFilterEntityType_Loan
			)
		AND (
			@IsAllLeveragedLease = 0 OR C.ContractType = @ContractFilterEntityType_LeveragedLease
			)
		AND (
			@InvoiceType = @InvoiceType_Unknown OR RCTG.InvoiceTypeId = @InvoiceTypeId
			)
		AND C.[Status] != @ContractStatusTerminated
		AND (
	...
	SELECT R.Id AS ReceivableId,
		...
		RR.InvoiceLeaddays AS CT_InvoiceLeadDays,
		RR.InvoiceTransitDays AS CT_InvoiceTransitDays,
...
END 		
	...
	SELECT IRDE.ReceivableDetailId,LSPS.PaymentType
	FROM InvoiceReceivableDetails_Extract IRDE
	JOIN LeasePaymentSchedules LSPS
		ON IRDE.PaymentScheduleId = LSPS.Id  AND LSPS.IsActive = 1 AND IRDE.ContractType = @ContractFilterEntityType_Lease
	...
	INSERT INTO #DownPaymentReceivableIds
	SELECT IRDE.ReceivableDetailId,LNPS.PaymentType
	FROM InvoiceReceivableDetails_Extract IRDE
	JOIN LoanPaymentSchedules LNPS
		ON IRDE.PaymentScheduleId = LNPS.Id AND LNPS.IsActive = 1 AND IRDE.ContractType=@ContractFilterEntityType_Loan
	...
	JOIN #DownPaymentReceivableIds ON #DownPaymentReceivableIds.ReceivableDetailId=InvoiceReceivableDetails_Extract.ReceivableDetailId
