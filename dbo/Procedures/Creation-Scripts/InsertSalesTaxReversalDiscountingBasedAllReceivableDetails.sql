...
CREATE PROCEDURE [dbo].[InsertSalesTaxReversalDiscountingBasedAllReceivableDetails]
(
...
INSERT INTO ReversalReceivableDetail_Extract (ReceivableTaxId, IsCashPosted,ReceivableTaxDetailId, ReceivableDetailId, Currency,TaxAreaId, Cost,
		...
		ReceivableTaxRowVersion, ReceivableTaxDetailRowVersion, AssetLocationRowVersion, JobStepInstanceId,CreatedById, CreatedTime, UpfrontTaxSundryId, 
		SalesTaxRemittanceResponsibility, IsAssessSalesTaxAtSKULevel, UpfrontTaxAssessedInLegacySystem, ReceivableTaxType, BusCode)
SELECT  RT.Id AS ReceivableTaxId
		...
		,CASE WHEN R.EntityType = @DTEntityType THEN R.EntityId ELSE NULL END AS DiscountingId
...
WHERE HasGLPeriod = 0 AND JobStepInstanceId = @JobStepInstanceId
UPDATE R
